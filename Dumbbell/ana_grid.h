...
!
      ...
      real(r8) :: dx, dy, f0, my_min, my_max, val1, fraction
...
!
      ...
      DO j=Jmin,Jmax
        DO i=Imin,Imax
          xp(i,j)=dx*REAL(i-1,r8) - Xsize*0.5
          xr(i,j)=dx*(REAL(i-1,r8)+0.5_r8) - Xsize*0.5
          xu(i,j)=xp(i,j)
          xv(i,j)=xr(i,j)
          yp(i,j)=dy*REAL(j-1,r8) - Esize*0.5
          yr(i,j)=dy*(REAL(j-1,r8)+0.5_r8) - Esize*0.5
          yu(i,j)=yr(i,j)
          yv(i,j)=yp(i,j)
        END DO
      END DO
